...
def KNN_training(X_train, y_train, CV_folds):
   ...
   # to implement a variant, selecting the simplest model within 1 SE of the best roc_auc_score
    ...
    for train_index, cv_index in kf.split(X_train):   
        ...
        model_results["confusion_matrix"]["tp"] += np.sum(np.logical_and(predicted == 1, true_labels == 1))
     
        # True Negative (TN): we predict a label of 0 (negative), and the true label is 0.
        model_results["confusion_matrix"]["tn"]  += np.sum(np.logical_and(predicted == 0, true_labels == 0))
     
        # False Positive (FP): we predict a label of 1 (positive), but the true label is 0.
        model_results["confusion_matrix"]["fp"]+= np.sum(np.logical_and(predicted == 1, true_labels == 0))
     
        # False Negative (FN): we predict a label of 0 (negative), but the true label is 1.
        model_results["confusion_matrix"]["fn"] += np.sum(np.logical_and(predicted == 0, true_labels == 1))
    ...
    for feature in features:
        if feature != outcome:
            if is_numeric_dtype(in_df[feature]):
                new_df = new_df.merge(in_df[feature], how='inner',left_index=True, right_index =True)
            else:
                dummy_set = pd.get_dummies(in_df[feature], prefix=feature, prefix_sep='_', dummy_na=True)
                new_df = new_df.merge(dummy_set, how='inner', left_index=True, right_index=True)
    return new_df

# split the data into training and test
def training_test_split(in_df, outcome, test_prop):
